...
function findMatches(user,sameZip, res, callback) {
  ...
  if (flag) {
    User.find({'_id': matchArray[0].user2}, function(err, u){
      //console.log('game', u);
      //var local = i;
      //match.user1game.push(g);
      matchArray[0].user2 = u;

      //skim out results that don't involve the calling user
      //console.log('this user', user._id)

      callback(null, matchArray);
      //res.status(200).json({error: 0, items: matches});
    });
  }  else{console.log("This line onyl prints if there are no matches");
  callback(null, []);}
  })
