...
module.exports = function(res, tradeIdArray) {
  ...
  async.each(tradeIdArray, function(tradeId, callback) {
    ...
    Trade.findById(tradeId, function(err, trade) {
      ...
      Game.findById(trade.gameId, function(err, game) {
        ...
        returnTrade.gameInfo = _.pick(game, ['_id', 'title', 'platform']);
        ...
        async.each(trade.potentialTrades, function(singleGame, done) {
          Game.findById(singleGame, function(err, ginfo) {
            ...
            returnTrade.potentialTrades.push(_.pick(ginfo, ['_id', 'title', 'platform']));
  ...
  function() {
    ...
    return helpers.returnSuccess(res, tradeInfoArray);
        ...
        // Game.find({_id: {$in: trade.potentialTrades}}, function(err, games) {

        //   _.forEach(games, function(item) {
        //     Game.findById(item._id, function(err, singleGame) {
        //       returnTrade.potentialTrades.push(_.pick(singleGame, ['_id', 'title', 'platform']));
        //     });
        //     console.log('infor',tradeInfoArray);
        //     //return returnTrade;
        //   });
        //   callback();
        // });

      // async.map(trade.potentialTrades, function(potential, callback) {
      //   Game.findById(trade.gameId, function(err, game) {
      //   callback(null, _.pick(game, ['_id', 'title', 'platform']));
      //   });
      // },
      // function(){
      ...
      //     });
      //   },
      //   //get info for potential trades
      //   function(callback){
      //     async.map(trade.potentialTrades, function(potential, done) {
      //       Game.findById(trade.gameId, function(err, game) {
      //         done(null, _.pick(game, ['_id', 'title', 'platform']));
      //       });
      //     });
      //     callback();
      //   }],
